...
for (let i = 0; i < stores.length; i += 1) {
  ...
  let storeState = store[2];
  let storeCity = store[4];
  ...
  if (storeCounts[storeName] === undefined) {
...
for (let storeName in storeCounts) {
  console.log(
    `There are ${storeCounts[storeName]} ${storeName} stores in total.`,
  );
}
